...
func send_kafka_location_event(topic string, partition string, location string) (bool, interface{}){
	...
	KafkaResponseEventChannel := make(chan kafka.Event, 10000) // event for listen for kafka event response...
	go kafka_client.ProduceEvent(KafkaResponseChannel, topic, partition, location)
